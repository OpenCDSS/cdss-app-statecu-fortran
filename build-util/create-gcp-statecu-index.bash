...
uploadIndexHtmlFile() {
  ...
  #         11  2019-04-27T10:01:42Z  gs://opencdss.state.co.us/statecu/14.0.0/software/
  #   94612246  2019-04-27T10:01:42Z  gs://opencdss.state.co.us/statecu/14.0.0/software/statecu-cdss-14.0.0.zip
  #
  # gs://opencdss.state.co.us/statecu/latest/software/:
  #   94612246  2019-04-27T10:01:42Z  gs://opencdss.state.co.us/statecu/14.0.0/software/statecu-cdss-14.0.0.zip
  ...
  #
  # 94612246  2019-04-27T10:01:42Z  gs://opencdss.state.co.us/statecu/14.0.0/software/statecu-cdss-14.0.0.zip
  # 94612246  2019-04-27T10:01:47Z  gs://opencdss.state.co.us/statecu/latest/software/statecu-cdss-14.0.0.zip

  # First do a listing of the software folder.
  tmpGcpSoftwareCatalogPath="/tmp/${USER}-statecu-software-catalog-ls.txt"
  ...
  if [ "${PIPESTATUS[0]}" -ne 0 ]; then
    echoStderr ""
    echoStderr "[Error] Error listing StateCU download files to create catalog."
    exit 1
  fi

  # Listing of the doc-user folders.
  tmpGcpDocUserCatalogPath="/tmp/${USER}-statecu-doc-user-catalog-ls.txt"
  ...
  if [ "${PIPESTATUS[0]}" -ne 0 ]; then
    echoStderr ""
    echoStderr "[Error] Error listing StateCU user documentation files to create catalog."
    exit 1
  fi

  # Listing of the doc-dev folders.
  tmpGcpDocDevCatalogPath="/tmp/${USER}-statecu-doc-dev-catalog-ls.txt"
  ...
  if [ "${PIPESTATUS[0]}" -ne 0 ]; then
    echoStderr ""
    echoStderr "[Error] Error listing StateCU developer documentation files to create catalog."
    exit 1
  fi

  # Create an index.html file for upload.
  indexHtmlTmpFile="/tmp/${USER}-statecu-index.html"
  gcpIndexHtmlUrl="${gcpFolderUrl}/index.html"

  # Also create index as CSV to facilitate listing available versions.
  indexCsvTmpFile="/tmp/${USER}-statecu-index.csv"
...
<title>OpenCDSS StateCU Downloads</title>
</head>
<body>
<h1>StateCU Software Downloads</h1>
<p>
<a href="https://opencdss.state.co.us/opencdss/statecu/">See also the OpenCDSS StateCU page</a>,
which provides additional information about StateCU.
</p>
<p>
<a href="https://www.colorado.gov/pacific/cdss/statecu">See also the CDSS StateCU page</a>,
which provides access to StateCU releases used in State of Colorado projects.
</p>
<p>
The StateCU software is available for Windows and Linux.
See the <a href="https://opencdss.state.co.us/statecu/latest/doc-user/">latest StateCU documentation</a>
for installation information (or follow a documentation link below for specific version documentation).
The StateCU software is typically packaged with the StateCU Graphical User Interface (StateMod GUI)
and can be included in the StateCU folder of datasets to ensure that a dataset is run with compatible version of StateCU.
</p>
<p>
<ul>
<li>Versions before 14 used two part version numbers (e.g., 13.10).
As of version 14, the version has three parts (e.g., 14.0.0).</li>
<li>The Windows installer zip files include the following files:
  <ul>
  <li>StateCU optimized executable, for general use (e.g.,
      <code>statecu-14.0.0-gfortran-win-64bit.exe</code>) - this file should be included in "StateCU" folder of datasets</li>
  <li>StateCU executable with runtime checks enabled, for troubleshooting
      (e.g., <code>statecu-14.0.0-gfortran-win-64bit-check.exe</code>) - this executable can be run during troubleshooting</li>
  <li>StateCU runner for Windows, which provides a generic name and runs the newest executable
      (<code>statecu.cmd</code>) - this file should be included in "StateCU" folder of datasets</li>
  <li>StateCU runner for Linux, which provides a generic name and runs the newest executable
      (<code>statecu</code>) - this file should be included in "StateCU" folder of datasets to
...
For example, copy to a model dataset "StateCU" folder.
Then run "statecu" to run a simulation.
</p>' > ${indexHtmlTmpFile}
...
<p>
Linux versions of StateCU are not currently provided by the State of Colorado.
...
uploadIndexHtmlFile_Table() {
  ...
  indexHtmlTmpCatalogFile="/tmp/${USER}-statecu-catalog-${downloadOs}.html"
  ...
  if [ "${catalogSource}" = "gcp" ]; then
    ...
    #
    # 12464143  2019-04-27T10:01:42Z  gs://opencdss.state.co.us/statecu/14.0.0/software/statecu-cdss-14.0.0.zip
    #
    ...
    cat "${tmpGcpSoftwareCatalogPath}" | grep "${downloadPattern}" | awk '{ printf "%s %s %s\n", $1, $2, $3 }' | sed -E 's|([0-9][0-9]/)|\1-zzz|g' | sed 's|/-zzz|-zzz|g' | sed 's|dev|-dev|g' | sort -r -k3,3 | sed 's|-zzz||g' | sed 's|-dev|dev|g' | awk -v debug=${debug} -v tmpGcpDocUserCatalogPath=${tmpGcpDocUserCatalogPath} -v tmpGcpDocDevCatalogPath=${tmpGcpDocDevCatalogPath} '
      ...
      }
      {
        ...
        # --- User documentation.
        cmd=sprintf("cat %s | grep 'statecu/%s/doc-user/index.html' | wc -l", tmpGcpDocUserCatalogPath, downloadFileVersion)
        ...
        if ( docCount > 0 ) {
          docUserUrl=sprintf("https://opencdss.state.co.us/statecu/%s/doc-user",downloadFileVersion)
        ...
        cmd=sprintf("cat %s | grep 'statecu/%s/doc-dev/index.html' | wc -l", tmpGcpDocDevCatalogPath, downloadFileVersion)
        cmd | getline docCount
        if ( debug == "true" ) {
        ...
        if ( docCount > 0 ) {
          docDevUrl=sprintf("https://opencdss.state.co.us/statecu/%s/doc-dev",downloadFileVersion)
...
gcpFolderUrl="gs://opencdss.state.co.us/statecu"
