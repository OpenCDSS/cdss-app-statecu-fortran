...
mainRepoName="cdss-app-statecu-fortran"
...
#mainRepoFolder="$HOME/cdss-dev/StateCU/git-repos/${mainRepoName}"
...
# Get the software version from the gcommon.inc file code:
#     PARAMETER (VERS = 13.10)
programVersionFile="${mainRepoFolder}/src/main/fortran/gcommon.inc"
if [ ! -f "${programVersionFile}" ]; then
	echo ""
	echo "StateCU version file does not exist:"
...
programVersion=`cat ${programVersionFile} | grep 'PARAMETER' | grep 'VERS' | cut -d '(' -f 2 | cut -d '=' -f 2 | tr -d ' ' | tr -d ')'`
echo ""
echo "StateCU version detected to be ${programVersion}"
...
# If it does not exist, create an output folder for the StateCU version
doxygenWorkFolder="${doxygenProjectFolder}/doxygen-statecu-${programVersion}"
doxygenWorkFolderRelative="doxygen-statecu-${programVersion}"
if [ ! -d "${doxygenWorkFolder}" ]; then
	echo ""
	echo "Creating Doxygen working folder for StateCU version:"
