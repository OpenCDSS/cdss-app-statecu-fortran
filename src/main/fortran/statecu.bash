...
#
# This is a generic script to run StateCU so that the user does not need to type the long executable name:
# - this script can be distributed with a StateCU dataset in the 'StateCU' folder,
#   with '.bash' extension removed
# - long executable name is similar to:  statecu-14.0.0-gfortran-win-64bit.exe
...
# Determine the StateCU executable.
# - echo the matched StateCU executable to stdout so it can be assigned in calling code
...
getStatecuExe() {
  ...
  #exeCount=$('ls' -1 ${statecuPattern} | sort --reverse | grep -v '32' | grep -v 'o3' | grep -v 'check' | wc -l 2> /dev/null)
  # The following lists the most recent version first.
  exeCount=$(listStatecuExes ${scriptFolder} | wc -l)
  echoStderr "Found ${exeCount} StateCU exectables.  Running the most recent version, based on sorting by version."
  if [ "${exeCount}" -eq 0 ]; then
    # Could not find a candidate executable.
    echoStderr "Could not find any production StateCU executables matching ${statecuPattern}"
    return 1
  else
    # List again and echo the first one, which should be the largest version:
    # - quote the command to disable alias
    #'ls' -1 ${statecuPattern} 2> /dev/null | sort --reverse | grep -v '32' | grep -v 'o3' | grep -v 'check' | head -1
    listStatecuExes ${scriptFolder} | head -1
    return 0
  fi
}

# List StateCU executables sorted by version number:
...
# - first parameter is the 'StateCU' dataset folder
...
listStatecuExes() {
  ...
  if [ ! -d "${statecuFolder}" ]; then
    ...
    echoStderr "StateCU folder does not exist: ${statecuFolder}"
    return 0
  fi
  ...
  find ${statecuFolder} -mindepth 1 -maxdepth 1 -type f -name '*statecu*exe' | awk -v debug=${debug} '
    BEGIN {
      if ( debug == "true" ) {
    ...
    #  /C/Users/user/cdss-dev/StateCU/git-repos/cdss-app-statecu-fortran-test/test/datasets/cm2015_StateCU/exes/statecu-14.0.0-gfortran-win-64bit/StateCU/statecu-14.0.0-gfortran-win-64bit.exe
    if ( debug == "true" ) {
...
# - do not implement command line parsing because don't want to confuse with StateCU options
...
# - the StateCU executable should be in the 'StateCU' folder of the dataset
echoStderr "Running in folder:"
echoStderr "  ${scriptFolder}"
cd ${scriptFolder}

statecuExe=$(getStatecuExe)
if [ $? -eq 0 ]; then
  # Success determining StateCU executable:
  # - run the program with the original command line options
  if [ -n "${statecuExe}" ]; then
    # Have an executable name.
    echoStderr ""
    echoStderr "Running StateCU executable:  '${statecuExe}' $@"
    echoStderr "If necessary, kill the process with: CTRL-c"
    echoStderr "If necessary, run a different StateCU executable using its file name."
    echoStderr ""
    ./${statecuExe} $@
    exit $?
  else
  ...
  # Error determining StateCU executable.
  printError
  exit 1
fi
